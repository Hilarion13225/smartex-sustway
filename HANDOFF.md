...
- **`GraphiqueLigne`** (évolution du score dans le temps) et le benchmark sectoriel : désormais portés, voir §3.9.
...
### 3.9 Évolution du score, benchmark sectoriel et 4 nouveaux widgets tableau de bord
Table `score_historique` (V17, une ligne par mission et par jour, UPSERT) alimentée par `ScoreHistoriqueService.enregistrer()`, appelé après chaque évaluation validée (`EvaluationResource.evaluer`, `RevueExperteResource.traiter`) — jamais depuis `AuditScoreService.calculer()` lui-même, qui reste un calcul pur sans effet de bord appelé à chaque lecture. `GET .../audits/{id}/score-historique` expose l'historique d'une mission ; `GET /secteurs/{code}/benchmark` renvoie la moyenne anonymisée du score le plus récent de chaque entreprise d'un secteur, avec un seuil de k-anonymat à 5 entreprises minimum (décision prise avec l'utilisateur : sous ce seuil, `scoreMoyen` reste `null` pour ne jamais révéler indirectement le score d'un concurrent). `EntrepriseDetail.jsx` affiche l'évolution + le benchmark (échelle d'une seule entreprise, donc d'un seul secteur — un portefeuille multi-entreprises mélangerait des secteurs différents). `TableauDeBord.jsx` gagne 4 widgets à partir de données déjà disponibles côté API : indice IFC/SFI moyen, répartition des niveaux d'engagement (`AuditScoreDto.repartitionNiveaux`, déjà calculée mais jamais affichée), portefeuille par entreprise et abonnements actifs (ces deux derniers visibles seulement au-delà d'une entreprise).
...
### 4.3 SUPER_ADMIN et ADMIN_AUDIT : accès global tranché avec l'utilisateur
Jusqu'ici, même SUPER_ADMIN avait besoin d'un rattachement `utilisateur_entreprise` explicite pour agir sur une entreprise donnée, comme n'importe quel autre rôle. **Tranché avec l'utilisateur : SUPER_ADMIN a un accès global complet (lecture ET écriture) à toutes les entreprises de la plateforme, sans rattachement préalable requis.** Implémenté via `AutorisationService.estAccesGlobalActif(utilisateurId)` (vrai dès qu'un rattachement, sur n'importe quelle entreprise, porte un rôle de `ROLES_ACCES_GLOBAL`) : toutes les méthodes de contrôle du service court-circuitent dessus, et `EntrepriseResource.mesEntreprises()` renvoie la liste complète des entreprises au lieu des seuls rattachements.

**Extension à ADMIN_AUDIT** : un seul compte ADMIN_AUDIT gère toutes les missions d'audit de tous les clients à ce stade du produit — exiger un rattachement entreprise par entreprise n'aurait aucun sens opérationnel (voir aussi §3.8, où l'affectation automatique/manuelle d'auditeurs a été retirée pour la même raison). `ROLES_ACCES_GLOBAL` contient donc désormais `SUPER_ADMIN` et `ADMIN_AUDIT`. EXPERT_REVIEWER reste volontairement borné à ses rattachements explicites — ce n'est pas ce rôle qui pilote les missions.
